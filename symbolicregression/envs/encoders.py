...
class FloatSequences(Encoder):
    def __init__(self, params):
        ...
        self.use_negative_constants = getattr(params, 'use_negative_constants', False)
        self.base = (self.float_precision + 1) // self.mantissa_len
        self.max_token = 10 ** self.base
        self.symbols = ["+", "-"]
        
        # Decide whether to add negative-constant tokens based on use_negative_constants
        if self.use_negative_constants:
            # Add positive and negative constants: N00-N99, -N00--N99
            self.symbols.extend(
            ...
            # Add only positive constants: N00-N99
            self.symbols.extend(
                ["N" + f"%0{self.base}d" % i for i in range(self.max_token)]
            )
        ...
        if len(values.shape) == 1:
            ...
            for val in value:
                ...
                if val in [-np.inf, np.inf]:
                    val = 1e100 if val == np.inf else -1e100
                sign = "+" if val >= 0 else "-"
