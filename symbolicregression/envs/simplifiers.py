...
class Simplifier:
    ...
    def tree_to_sympy_expr(self, tree):
        ...
        # Check whether exp is combined with large values, which can produce extremely small or large coefficients
        # If so, clamp the numeric range to [-10, 10] to avoid exp overflow
        if 'exp' in prefix:
            modified_prefix = []
            for token in prefix:
                try:
                    val = float(token)
                    # Limit the absolute value to at most 10 to avoid exp(±large) producing extremely small or large coefficients
                    if abs(val) > 10:
                        if val > 0:
                            modified_prefix.append('10.0')
                        else:
                            modified_prefix.append('-10.0')
                    else:
                        modified_prefix.append(token)
                except ValueError:
                    modified_prefix.append(token)
