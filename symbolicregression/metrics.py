...
def compute_metrics(infos, metrics="r2"):
    ...
    for metric in metrics.split(","):
        if metric == "r2":
            true, predicted = infos["true"], infos["predicted"]
            for i in range(len(true)):
                # Convert to numpy arrays to avoid PyTorch tensor issues
                pred_i = np.asarray(predicted[i])
                true_i = np.asarray(true[i])
                if predicted[i] is None or np.isnan(np.min(pred_i)):
                    results[metric].append(np.nan)
                else:
                    try:
                        results[metric].append(r2_score(true_i, pred_i))
        ...
        if metric == "r2_zero":
            ...
            for i in range(len(true)):
                ...
                if predicted[i] is None or np.isnan(np.min(pred_i)):
                    ...
                    try:
                        results[metric].append(max(0, r2_score(true_i, pred_i)))
                    except Exception as e:
                        results[metric].append(np.nan)
