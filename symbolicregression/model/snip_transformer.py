...
class SNIP_TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        self.n_layers = params.n_enc_layers if is_encoder else params.n_dec_layers # Default was 2 for encoder/16 for decoder. Modified: Both 8
    ...
    def fwd(
        ...
        Inputs:
            ...
            `return_features` Boolean, if True, return token-wise features before pooling (for REPA)
        """
        ...
        ### modified (Src masking is not needed for input sequence)
        src_mask = None 
        ...
        for i in range(self.n_layers):
            ...
            if self.is_decoder and src_enc is not None:
                ...
                ### Modified
                # map z_rep to hidden size
                src_enc_proj = self.z_projection(src_enc)  # shape: (batch_size, hidden_size)
                # expand z_rep to match shape of encoder hidden states
                src_enc_expanded = src_enc_proj.unsqueeze(1).expand(tensor.shape)  # shape: (seq_len, batch_size, hidden_size)
                ####
        ...
        if self.is_encoder:
            # Capture features before global attention pooling (for REPA)
            features = tensor.clone()  # (B, S, Dim)
            ...
            if return_features:
                # Return both pooled representation and token-wise features
                return z_rep, features
            return z_rep
    ...
    def generate(
        ...
        return generated, gen_len #modified
    ...
    def generate_from_latent(
        ...
        # generated sentences
        generated = src_enc.new(max_len, bs).long()  # upcoming output
        generated.fill_(self.pad_index)  # fill upcoming ouput with <PAD>
        generated[0].fill_(self.eos_index)  # we use <EOS> for <BOS> everywhere
...
class SNIP_E2E_MAP(nn.Module):
    def __init__(
        self,
        params,
    ):
        """
        Mapping from encoder latent vector space to E2E Decoder src_enc.
        """
        super().__init__()
        self.params = params
        ...
        self.linear1 = nn.Linear(self.latent_dim, self.seq_len_dec * self.dec_emb_dim)
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(self.dec_emb_dim, self.dec_emb_dim)

    def forward(
        self,
        z_enc,
    ):
        # Apply the mapping layers
        x = self.linear1(z_enc)
        x = self.relu(x)
        x = x.view(-1, self.seq_len_dec, self.dec_emb_dim)  # Reshape
        src_enc = self.linear2(x)
