...
class BFGSRefinement():
    ...
    def go(
        ...
        def objective_numpy(coeffs):
            ...
            """
            val = objective_torch(coeffs).item()
        ...
        def gradient_numpy(coeffs):
            ...
            try:
                ...
                grad_np = grad_obj.detach().numpy()
                if np.any(np.isnan(grad_np)) or np.any(np.isinf(grad_np)):
                    return np.zeros_like(grad_np)
                return grad_np
            except Exception:
                return np.zeros_like(coeffs.detach().numpy())
        ...
        if best_constants is None or np.any(np.isnan(best_constants)):
            return None
