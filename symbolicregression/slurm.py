...
def init_distributed_mode(params):
    ...
    # multi-GPU job (local or multi-node) - jobs started with torch.distributed.launch
    elif params.local_rank != -1 or "RANK" in os.environ:
        ...
        if "NGPU" in os.environ:
            ...
            if hasattr(torch, "npu") and torch.npu.is_available():
                params.n_gpu_per_node = torch.npu.device_count()
            else:
    ...
    if params.multi_gpu:
        ...
        if hasattr(torch, "npu") and torch.npu.is_available():
            backend = "hccl"
            # Set HCCL timeout to 3 hours (10800s) to avoid transient timeouts in large jobs
            if "HCCL_EXEC_TIMEOUT" not in os.environ:
                os.environ["HCCL_EXEC_TIMEOUT"] = "10800"
                logger.info("Setting HCCL_EXEC_TIMEOUT to 10800s")
        
        torch.distributed.init_process_group(
            init_method="env://", backend=backend,
        )
